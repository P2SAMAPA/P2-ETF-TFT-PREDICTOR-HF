...
# --- 3. ORIGINAL HIGH-CAPACITY ARCHITECTURE ---
class MomentumTransformer(nn.Module):
    def __init__(self, input_dim, d_model=64, num_heads=8, seq_len=30): # Restored depth
        ...
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=3) # Restored layers
...
def train_engine(start_year, tx_cost):
    ...
    # Load full history but train based on anchor
    data = sync_data_persistent(etfs, "2008-01-01")
    ...
    # Correct OOS Logic: Train on pre-anchor, Test on post-anchor
    anchor_date = pd.Timestamp(f"{start_year}-12-31")
    train_idx = features_df.index[features_df.index <= anchor_date]
    oos_idx = features_df.index[features_df.index > anchor_date]
    
    scaler = StandardScaler()
    scaled_train = scaler.fit_transform(features_df.loc[train_idx].astype(np.float64))
    scaled_oos = scaler.transform(features_df.loc[oos_idx].astype(np.float64))
    
    X_train = [scaled_train[i:i+30] for i in range(len(scaled_train)-30)]
    y_train = torch.FloatTensor(target_df.loc[train_idx].values[30:])
    X_train = torch.FloatTensor(np.array(X_train))

    model = MomentumTransformer(input_dim=features_df.shape[1])
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0005) # Slower, more precise learning
    loss_log = []
    
    model.train()
    for e in range(100): # Original training depth
        optimizer.zero_grad()
        output = model(X_train)
        loss = nn.MSELoss()(output, y_train[:len(output)])
        loss.backward(); optimizer.step(); loss_log.append(loss.item())
        
    return {"model": model, "scaler": scaler, "returns": returns_df[etfs+['CASH']], "oos_features": scaled_oos, "loss": loss_log, "sofr": sofr_val, "oos_dates": oos_idx}
...
with m2:
    ...
    st.markdown(f"**Ann. Return**\n<h2 style='margin:0;'>{ann_ret:.1f}%</h2><p style='font-size:12px; color:gray;'>{oos_yrs} Years OOS</p>", unsafe_allow_html=True)
with m3:
...
st.info(f"""
**Seeded Deterministic Output**: This model is explicitly seeded (Seed: 42) to ensure consistent, reproducible predictions across sessions.
- **Original Architecture**: Reinstated 3-layer Transformer with 8-head attention for high-capacity pattern recognition.
- **Dynamic OOS Alignment**: The Out-of-Sample period is dynamically calculated based on the Year Anchor ({regime_year}).
- **Self-Updating Data**: Datasets are automatically synced daily from live financial feeds via the Hugging Face Parquet cache.
""")
