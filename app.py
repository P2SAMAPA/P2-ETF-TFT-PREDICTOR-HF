...
try:
    from models import train_all_binary_tfts, predict_binary_tfts, SEED
except Exception as _e:
    st.error(f"❌ models.py import failed: {_e}")
    st.stop()
...
if run_button:
    ...
    # ── Fetch risk-free rate first (needed for binary target threshold) ────────
    sofr     = 0.045
    ...
    # ── Binary targets: 1 if ETF 5-day return > cash return over same period ──
    ...
    # ── Auto-optimise lookback (20/30/40/50/60 days) ─────────────────────────
    LOOKBACK_CANDIDATES = [20, 30, 40, 50, 60]
    best_lookback   = 30
    best_val_loss   = float('inf')
    lookback_results = {}
    ...
    with st.status("🔍 Auto-optimising lookback window...", expanded=False) as status:
        for lb in LOOKBACK_CANDIDATES:
            X_lb, y_lb = [], []
            # Use first ETF as proxy for lookback search
            daily_rf_5d = (sofr / 252) * FORWARD_DAYS
            bin_proxy = (fwd_model[target_etfs[0]] > daily_rf_5d).astype(int).values
            ...
            from models import build_binary_tft, SEED
            import tensorflow as tf
            import random as _random
            _random.seed(SEED); np.random.seed(SEED); tf.random.set_seed(SEED)

            probe_model = build_binary_tft(seq_len=lb, num_features=X_lb.shape[2])
            probe_model.compile(
    ...
    bin_vals      = binary_targets[target_etfs].values  # (N, n_etfs) binary
    fwd_vals      = fwd_model[target_etfs].values       # (N, n_etfs) fwd returns
    ...
    for i in range(lookback, len(scaled) - 1):
        ...
        model_date = df_model_dates[i + 1]
        dates_seq.append(model_date)

    X     = np.array(X,     dtype=np.float32)
    y_bin = np.array(y_bin, dtype=np.int32)    # (N, n_etfs) binary
    y_fwd = np.array(y_fwd, dtype=np.float32)  # (N, n_etfs) fwd returns
    ...
    # Extend test_dates with dates in df after df_model ends
    ...
    with st.spinner(f"🧠 Training {len(target_etfs)} Binary TFTs (one per ETF, early stopping active)..."):
        ...
        epochs_per = [len(h.history['loss']) for h in histories]
        st.success(f"✅ Training complete — epochs: {dict(zip(etf_names, epochs_per))}")

    # ── Predict: P(beat cash) for each ETF ───────────────────────────────────
    proba = predict_binary_tfts(models, X_test)   # (N, n_etfs)
    ...
    # ── Accuracy diagnostic (per ETF binary accuracy) ─────────────────────────
    orig_n = len(X_test)
    acc_per_etf = {}
    for j, name in enumerate(etf_names):
        preds_j = (proba[:orig_n, j] > 0.5).astype(int)
        acc_per_etf[name] = f"{np.mean(preds_j == y_bin_test[:, j]):.1%}"
    st.info(f"🎯 **Binary Accuracy per ETF:** {acc_per_etf} | Random baseline: 50.0%")
    # ── Daily returns for P&L — use df (full dataset) ────────────────────────
    ...
    At inference time, all ETFs that the model expects to beat cash are ranked by their
    confidence probability — the highest-confidence ETF is selected for next-day execution.</p>

    <b style="color:#00d1b2;">Why binary classifiers beat a 5-class model:</b>
    <ul>
      <li>Each model focuses on one ETF's specific patterns rather than competing across all 5</li>
      <li>Avoids regime-lock where one ETF dominates the softmax output</li>
      <li>Random baseline is 50% (much easier to beat than 20% for 5-class)</li>
      <li>If no ETF is predicted to beat cash, the strategy defaults to CASH — natural filter</li>
    </ul>
