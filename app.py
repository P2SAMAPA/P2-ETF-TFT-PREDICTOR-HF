...
import os
import interface  # <--- WE IMPORT YOUR UI FILE
...
# Inference & Tactical Override Logic
agent.eval()
...
for i, ticker in enumerate(etf_universe):
    if returns[ticker].tail(2).sum() < -0.015: continue # The "Switch" Logic
    for h in [1, 3, 5]:
...
audit_df = pd.DataFrame({"Date": returns[top_pick].tail(15).index.strftime('%Y-%m-%d'), "Ticker": [top_pick]*15, "Net Return": [f"{v:.2%}" for v in returns[top_pick].tail(15).values]})

# Render UI from the Locked File
interface.render_main_output(top_pick, "1.82", hit_rate, top_horizon, wealth, audit_df)
