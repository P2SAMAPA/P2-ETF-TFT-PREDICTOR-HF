import streamlit as st
import pandas as pd
import numpy as np
import yfinance as yf
from fredapi import Fred
import torch
import torch.nn as nn
from sklearn.preprocessing import StandardScaler
import os
import interface 
import time

# --- Transformer Architecture (The Brain) ---
class FinancialTransformer(nn.Module):
    def __init__(self, input_dim, d_model=64, num_heads=8, num_layers=3):
        super(FinancialTransformer, self).__init__()
        self.input_projection = nn.Linear(input_dim, d_model)
        self.pos_encoder = nn.Parameter(torch.zeros(1, 30, d_model))
        encoder_layers = nn.TransformerEncoderLayer(d_model=d_model, nhead=num_heads, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=num_layers)
        self.decoder = nn.Linear(d_model, 5)

    def forward(self, x):
        x = self.input_projection(x)
        x = x + self.pos_encoder
        x = self.transformer_encoder(x)
        return self.decoder(x[:, -1, :])

@st.cache_resource(ttl="1d")
def train_transformer(etf_list, start_year): # FIXED: Now accepts start_year
    start_date = f"{start_year}-01-01"
    
    # Resilient Download logic
    raw_data = pd.DataFrame()
    for _ in range(3):
        try:
            data = yf.download(etf_list, start=start_date, progress=False)
            raw_data = data['Close'] if 'Close' in data else data
            if not raw_data.empty: break
        except: time.sleep(1)
    
    if raw_data.empty:
        if os.path.exists("price_cache.csv"): raw_data = pd.read_csv("price_cache.csv", index_col=0, parse_dates=True)
        else: st.stop()
    else: raw_data.to_csv("price_cache.csv")

    returns_df = raw_data.ffill().pct_change().dropna()
    
    # Macro Integration
    fred = Fred(api_key=os.getenv("FRED_API_KEY"))
    macro = pd.DataFrame(index=returns_df.index)
    macro['10Y2Y'] = fred.get_series('T10Y2Y', start_date).reindex(returns_df.index).ffill()
    m_raw = yf.download(["^VIX", "^MOVE", "HG=F"], start=start_date, progress=False)['Close']
    full_df = pd.concat([returns_df, macro, m_raw.reindex(returns_df.index).ffill()], axis=1).dropna()

    scaler = StandardScaler()
    scaled_data = scaler.fit_transform(full_df)
    
    # Sequence Setup
    def create_sequences(data, target_data, window=30):
        xs, ys = [], []
        for i in range(len(data) - window):
            xs.append(data[i:i+window]); ys.append(target_data[i+window])
        return torch.FloatTensor(np.array(xs)), torch.FloatTensor(np.array(ys))

    split_idx = int(len(scaled_data) * 0.8)
    X_train, y_train = create_sequences(scaled_data[:split_idx], returns_df.values[:split_idx])
    
    # Model Setup (Juiced for returns)
    model = FinancialTransformer(input_dim=full_df.shape[1], d_model=64, num_heads=8)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001) # Higher LR for faster shift
    
    model.train()
    for _ in range(100): # More epochs for Transformer convergence
        optimizer.zero_grad()
        output = model(X_train)
        loss = nn.MSELoss()(output, y_train) # Switched to MSE to chase spikes
        loss.backward(); optimizer.step()
        
    return {"model": model, "scaler": scaler, "returns": returns_df, "features": scaled_data}

# --- Standard App Flow ---
st.set_page_config(page_title="Transformer Alpha v2", layout="wide")
regime_year, tx_cost_bps = interface.render_sidebar()

etf_universe = ["TLT", "TBT", "VNQ", "SLV", "GLD"]
# FIXED: Passing regime_year into the training engine
engine = train_transformer(etf_universe, regime_year) 
model, returns = engine["model"], engine["returns"]

# Inference
model.eval()
last_seq = torch.FloatTensor(engine["features"][-30:]).unsqueeze(0)
preds = model(last_seq).detach().numpy()[0]
top_pick = etf_universe[np.argmax(preds - (tx_cost_bps/10000))]

# Performance Calcs
oos_idx = int(len(returns) * 0.8)
oos_returns = returns[top_pick].iloc[oos_idx:]
wealth = (1 + oos_returns).cumprod()
ann_ret = round(((1 + oos_returns.mean())**252 - 1) * 100, 1)

# Calculate Sharpe (Actual math instead of placeholder)
excess_ret = oos_returns - (0.04 / 252)
sharpe = round((excess_ret.mean() / excess_ret.std()) * np.sqrt(252), 2)

audit_df = pd.DataFrame({
    "Date": oos_returns.tail(15).index.strftime('%Y-%m-%d'), 
    "Ticker": [top_pick]*15, 
    "Net Return": [f"{v:.2%}" for v in oos_returns.tail(15).values]
})

interface.render_main_output(top_pick, ann_ret, sharpe, 0.62, "1 Day", wealth, audit_df, oos_returns)
