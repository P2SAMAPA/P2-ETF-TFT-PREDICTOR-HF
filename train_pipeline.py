    ...
    for attempt in range(max_retries):
        try:
            ...
            log.info(f"✅ Pushed {filename} → {HF_OUTPUT_REPO} ({len(content_bytes):,} bytes)")
                ...
                HfApi().upload_file(path_or_fileobj=tmp.name, path_in_repo=filename,
                                    repo_id=HF_OUTPUT_REPO, repo_type="dataset",
                                    token=token, commit_message=commit_msg)
            ...
            if e.response.status_code in (412, 429):
                ...
                wait = 2**attempt
                log.warning(f"Upload failed {e.response.status_code}, retry {wait}s")
                time.sleep(wait)
            else:
                raise
...
def download_file_from_hf_dataset(filename, token):
    ...
    return local_path
    return hf_hub_download(repo_id=HF_OUTPUT_REPO, repo_type="dataset", filename=filename, token=token)
...
def prepare_data(option, start_year, force_refresh):
    ...
    input_features = [c for c in df.columns if (c.endswith('_Z') or c.endswith('_Vol') or 'Regime' in c or 'YC_' in c or 'Credit_' in c or 'Rates_' in c or 'VIX_Term_' in c or 'Rising' in c or 'Falling' in c or 'Accelerating' in c) and c not in target_etfs]
...
@@ -176,438 +101,259 @@ def optimize_lookback(scaled, bin_proxy, lookback_candidates, seed):
    lookback_results = {}
    for lb in lookback_candidates:
        ...
        probe.compile(optimizer=tf.keras.optimizers.Adam(5e-4), loss='binary_crossentropy')
        h = probe.fit(X_lb[:ts], y_lb[:ts], validation_data=(X_lb[ts:ts+vs], y_lb[ts:ts+vs]),
                      epochs=20, batch_size=64, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)], verbose=0)
    ...
    log.info(f"Best lookback: {best_lookback}d")
            best_val_loss, best_lookback = vl, lb
...
def create_sequences(scaled, bin_vals, fwd_vals, dates, lookback, seed):
    ...
    val_size = int(len(X) * VAL_PCT)
    return (X[:train_size], y_bin[:train_size],
            X[train_size:train_size+val_size], y_bin[train_size:train_size+val_size],
            X[train_size+val_size:], y_fwd[train_size+val_size:], y_bin[train_size+val_size:],
            dates_seq[train_size+val_size:])
...
def train_models(X_train, y_bin_train, X_val, y_bin_val, etf_names, epochs=150, seed=42):
    ...
    models, histories = train_all_binary_tfts(X_train, y_bin_train, X_val, y_bin_val, etf_names=etf_names, epochs=epochs)
...
def compute_strategy_metrics(models, X_test, y_bin_test, y_fwd_test, test_dates, target_etfs, sofr, daily_ret):
...
# ── Global model functions ────────────────────────────────────────────────────
    acc_per_etf = {name: round(float(np.mean((proba[:,j] > 0.5).astype(int) == y_bin_test[:,j])), 4) for j, name in enumerate(target_etfs)}
    strat_rets, _, _, _, _, _, _ = execute_strategy(proba, y_fwd_test, test_dates, target_etfs,
                                                    fee_bps=15, stop_loss_pct=-0.12, z_reentry=1.0,
                                                    sofr=sofr, z_min_entry=0.5, daily_ret_override=daily_ret)
    metrics = calculate_metrics(strat_rets, sofr)
    return proba, acc_per_etf, round(metrics['ann_return'],6), round(metrics['sharpe'],6), round(metrics['max_dd'],6), strat_rets
# ========== CORRECTED SAVE FUNCTION – NO ROOT FOLDER ==========
def save_global_model(models, scaler, lookback, lookback_results, target_etfs, input_features, option, token):
    import tensorflow as tf
    import pickle
    import tempfile
    import tensorflow as tf, pickle, tempfile, os
    from huggingface_hub import HfApi
    import os
    with tempfile.TemporaryDirectory() as tmpdir:
        # Build correct structure: option_{option}/global_model/
        # Build correct path: option_{option}/global_model/
        option_dir = os.path.join(tmpdir, f"option_{option}")
        local_root = os.path.join(option_dir, "global_model")
        os.makedirs(local_root, exist_ok=True)
        global_dir = os.path.join(option_dir, "global_model")
        os.makedirs(global_dir, exist_ok=True)
        # Save weights
        for etf, model in zip(target_etfs, models):
            weights_path = os.path.join(local_root, f"{etf}.weights.h5")
            model.save_weights(weights_path)
            model.save_weights(os.path.join(global_dir, f"{etf}.weights.h5"))
        # Save scaler
        ...
        meta = {"lookback": lookback, "lookback_results": lookback_results,
                "target_etfs": target_etfs, "input_features": input_features,
                "num_features": len(input_features),
                "run_timestamp_utc": datetime.now(timezone.utc).isoformat()}
        ...
        # Upload ONLY the option_{option} folder – no root global_model
        HfApi().upload_folder(repo_id=HF_OUTPUT_REPO, repo_type="dataset",
                              folder_path=option_dir, path_in_repo="",
                              commit_message=f"Global model {option} {datetime.now().strftime('%Y-%m-%d')}",
                              token=token)
        log.info(f"✅ Uploaded global model for option {option}")
...
def train_global(option, force_refresh, token):
    ...
    df, df_model, fwd_model, binary_targets, target_etfs, input_features, sofr, rf_label = prepare_data(option, 2008, force_refresh)
    scaler = RobustScaler().fit(df_model[input_features].values)
    ...
    best_lookback, lookback_results = optimize_lookback(scaled, bin_proxy, [20,30,40,50,60], SEED)
    ...
    X_train, y_bin_train, X_val, y_bin_val, X_test, y_fwd_test, y_bin_test, test_dates = create_sequences(scaled, bin_vals, fwd_vals, df_model.index, best_lookback, SEED)
    etf_names = [e.replace('_Ret','') for e in target_etfs]
    models, _ = train_models(X_train, y_bin_train, X_val, y_bin_val, etf_names, epochs=150, seed=SEED)
    ...
    proba, acc, ann_return, sharpe, max_dd, _ = compute_strategy_metrics(models, X_test, y_bin_test, y_fwd_test, test_dates, etf_names, sofr, daily_ret_test)
    log.info(f"Global metrics: AnnReturn={ann_return*100:.2f}%, Sharpe={sharpe:.2f}, MaxDD={max_dd*100:.2f}%")
    ...
    signals_payload = {"next_signal": next_signal, "next_date": str(get_next_trading_day(test_dates[-1])),
                       "conviction_z": float((last_scores[best_idx]-0.5)*2),
                       "conviction_label": "High" if last_scores[best_idx]>0.7 else "Medium" if last_scores[best_idx]>0.55 else "Low",
                       "etf_scores": {name: float(score) for name,score in zip(etf_names, last_scores)},
                       "sofr": sofr, "rf_label": rf_label, "data_start": str(df.index[0].date()), "data_end": str(df.index[-1].date()),
                       "test_start": str(test_dates[0].date()), "test_end": str(test_dates[-1].date()), "n_test_days": len(test_dates),
                       "lookback_days": best_lookback, "start_year": 2008, "ann_return": ann_return, "sharpe": sharpe, "max_dd": max_dd,
                       "run_timestamp_utc": datetime.now(timezone.utc).isoformat()}
    push_file_to_hf_dataset(f"option_{option}/global_model/signals.json", json.dumps(signals_payload, indent=2).encode(),
                            f"Global signals {datetime.now().strftime('%Y-%m-%d')}", token)
...
def predict_global(option, year, sweep_date, force_refresh, token):
    ...
    df_full, df_model, fwd_model, binary_targets, _, _, sofr, rf_label = prepare_data(
        option, start_year=2008, force_refresh=force_refresh)
    df_full, df_model, fwd_model, binary_targets, _, _, sofr, rf_label = prepare_data(option, 2008, force_refresh)
    ...
    sharpe_val     = round(float(strat_metrics['sharpe']),     6)
    max_dd_val     = round(float(strat_metrics['max_dd']),     6)
    strat_rets, _, _, _, _, _, _ = execute_strategy(proba, y_fwd_test, test_dates_year, target_etfs,
                                                    fee_bps=15, stop_loss_pct=-0.12, z_reentry=1.0,
                                                    sofr=sofr, z_min_entry=0.5, daily_ret_override=daily_ret_test)
    metrics = calculate_metrics(strat_rets, sofr)
    ann_return_val = round(metrics['ann_return'],6)
    sharpe_val = round(metrics['sharpe'],6)
    max_dd_val = round(metrics['max_dd'],6)
    last_scores = proba[-1]
    best_idx = np.argmax(last_scores)
    ...
    conviction_z = float((last_scores[best_idx]-0.5)*2)
    conviction_label = "High" if last_scores[best_idx]>0.7 else "Medium" if last_scores[best_idx]>0.55 else "Low"
    sweep_data = {"next_signal": next_signal, "conviction_z": conviction_z, "conviction_label": conviction_label,
                  "ann_return": ann_return_val, "sharpe": sharpe_val, "max_dd": max_dd_val,
                  "lookback_days": lookback, "start_year": year, "sweep_date": sweep_date,
                  "etf_scores": {name: float(score) for name,score in zip(target_etfs, last_scores)},
                  "model_type": "global"}
    sweep_fname = f"global_sweep/option_{option}/signals_{year}_{sweep_date}.json"
    push_file_to_hf_dataset(sweep_fname, json.dumps(sweep_data, indent=2).encode(),
                            f"[global sweep] {year} {sweep_date} → {next_signal}", token)
...
def train_year(option, force_refresh, start_year, sweep_date, token):
    ...
    df, df_model, fwd_model, binary_targets, target_etfs, input_features, sofr, rf_label = prepare_data(option, start_year, force_refresh)
    scaler = RobustScaler()
    ...
    proba, acc_per_etf, ann_return_val, sharpe_val, max_dd_val, strat_rets = compute_strategy_metrics(
        models, X_test, y_bin_test, y_fwd_test, test_dates, etf_names, sofr, daily_ret_test)
    proba, acc, ann_return_val, sharpe_val, max_dd_val, _ = compute_strategy_metrics(models, X_test, y_bin_test, y_fwd_test, test_dates, etf_names, sofr, daily_ret_test)
    last_scores = proba[-1]
    ...
    sweep_data = {"next_signal": next_signal, "conviction_z": round(conviction_z,4), "conviction_label": conviction_label,
                  "ann_return": ann_return_val, "sharpe": sharpe_val, "max_dd": max_dd_val,
                  "lookback_days": best_lookback, "start_year": start_year, "sweep_date": sweep_date,
                  "etf_scores": {name: round(float(score),4) for name,score in zip(etf_names, last_scores)}}
    sweep_fname = f"sweep/option_{option}/signals_{start_year}_{sweep_date}.json"
    push_file_to_hf_dataset(sweep_fname, json.dumps(sweep_data, indent=2).encode(),
